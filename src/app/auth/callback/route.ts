 ...
 * et eksisterende), og sender brukeren videre til `next`. Feiler pent til
 * /logg-inn?feil=google — aldri en hvit skjerm (også når Google-provideren
 * ikke er satt opp i Supabase ennå).
 */
  ...
  if (code) {
    ...
    if (!error) {
      ...
      try {
        ...
        if (user && !eksisterende && fraGoogle) {
          await supabase.auth.updateUser({ data: { fornavn: fraGoogle } });
        }
      } catch (feil) {
        console.error("Google fornavn-berikelse feilet", feil);
      }
